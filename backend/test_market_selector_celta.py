...
Diagnóstico completo do mapeamento de mercados e probabilidades
"""
...
if not match:
    print("❌ Match Celta vs Osasuna não encontrado")
    exit(1)
...
# Simular dados de análise (baseado na estrutura do teste anterior)
...
print(f"\n📊 PADRÕES DETECTADOS: {len(context_analysis.get('patterns', []))}")
...
print("\n" + "="*80)
print("2️⃣ RODANDO MARKET SELECTOR - VALUE BET")
print("="*80)
...
for bet in selected_value:
    ...
    print(f"      Razão: {bet['reasoning']}")

# 3. RODAR MARKET SELECTOR - MULTIPLE
print("\n" + "="*80)
print("3️⃣ RODANDO MARKET SELECTOR - BILHETE MÚLTIPLO")
print("="*80)
...
print("\n" + "="*80)
print("4️⃣ DIAGNÓSTICO DE MAPEAMENTO DE MERCADOS")
print("="*80)

print("\n📋 MERCADOS DO CONTEXTO vs PROBABILIDADES DISPONÍVEIS:")
...
for market_data in context_analysis.get('top_markets', [])[:10]:
    ...
    if prob > 0:
        ...
        print(f"      ❌ Probabilidade NÃO encontrada")
        print(f"         Tentando variações:")
        # Tentar variações
        variations = [
            market,
            market.replace('_', ''),
...
print("\n" + "="*80)
print("📝 PROBABILIDADES DISPONÍVEIS NO MODELO:")
print("="*80)
